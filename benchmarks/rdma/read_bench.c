...
inline void postRead(void* memAddr, size_t size, struct ibv_qp* qp, struct ibv_mr* mr, enum completion wc, size_t rkey, size_t remoteOffset, size_t wcId, bool needFence, int batch_size)
{
  ...
  for (int i = 0; i < batch_size; i++) {
    sq_wr[i].opcode = IBV_WR_RDMA_READ;
  ...
  if (ret) {
    printf("FAILED: Submit read failed\n");  
    exit(EXIT_FAILURE);
  }
}
  ...
  *total_ops = 0;
  ...
  while(diff < targ->cycles) {
    for (int i = 0; i < num_qps; i++) {
      postRead(ctx[i]->local_buf, rd_sz, ctx[i]->id->qp, ctx[i]->mr, is_signaled, ctx[i]->mem_info.rkey, ctx[i]->mem_info.addr, 0, need_fence, batch_size);
    }
...
const char *argp_program_version = "read_bench 1.0";
const char *argp_program_bug_address = "ashfaq@cs.utah.edu";

/* Program documentation. */
static char doc[] = "RDMA READ Benchmark Client\n"
"This program connects to a RDMA server and performs RDMA READs.\n"
"It measures throughput and avg. latency of the read operations.";
  ...
  switch (key) {
    ...
    case OPT_OBJECT_SIZE:
      ...
      if (arguments->object_size <= 0) {
        argp_error(state, "read_size must be a positive integer");
      }
      break;
