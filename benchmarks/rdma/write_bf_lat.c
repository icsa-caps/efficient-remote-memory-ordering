...
inline void postWrite(void* memAddr, size_t size, struct ibv_qp* qp, struct ibv_mr* mr, enum completion wc, size_t rkey, size_t remoteOffset, size_t wcId, bool needFence, int batch_size)
{
  ...
  send_sgl[1].addr = (uint64_t)(unsigned long)memAddr + 1024;
  send_sgl[1].length = size;
  send_sgl[1].lkey = mr->lkey;
...
void *run_benchmark(void *arg) {
  ...
  *total_ops = 0;
  ...
  if (!samples)
    die("malloc");
  memset(samples, 0, MAX_SAMPLES * sizeof(uint64_t));

  uint64_t sample_idx = 0;
  while(diff < targ->cycles) {
    uint64_t start_op = rdtsc();
    postWrite(ctx[0]->local_buf, rd_sz, ctx[0]->id->qp, ctx[0]->mr, signaled, ctx[0]->mem_info.rkey, ctx[0]->mem_info.addr, 0, false, 1);

    int comp = 0;
    int tot_comp = 0;
    int tot_expected = 1;
    struct ibv_wc wcReturn[1];
    while (tot_comp != tot_expected) {
      _mm_pause();
      int expected = tot_expected - tot_comp;
      comp = pollCompletion(ctx[0]->id->qp->send_cq, expected, wcReturn);
    ...
    uint64_t end_op = rdtsc();
    samples[sample_idx++] = end_op - start_op;

    *total_ops += 1;
    diff = rdtsc() - start;
  }

  // conver latencies to ns
  for (uint64_t i = 0; i < sample_idx; i++) {
    samples[i] = (uint64_t)((double)samples[i] * 1e9 / tsc_freq);
  }
...
static struct argp_option options[] = {
  ...
  {"outfile", OPT_OUTFILE, "FILE", 0, "Output CSV file path (default: results/write_bf_lat.csv)" },
  { 0 }
};
...
int main(int argc, char **argv) {
  struct arguments arguments = {16, 64, 1, 1, false, SERVER_IP, SERVER_PORT, "results/write_bf_lat.csv"};
